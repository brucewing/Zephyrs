...
// Get myUnits file
        SharedPreferences spMyUnits = getActivity().getSharedPreferences("myUnits",
                Context.MODE_PRIVATE); switch (v.getId()) {
            // If display units button is clicked:
            case R.id.b_display_units:
// Gather user input
                String year = sYear.getSelectedItem().toString(); String semester;
                if (rbSem1.isChecked()) {
                    semester = "Sem 1";
...
// Search for corresponding units and save matching results to
                ...
                if (jaSearchUnits.length() == 0) { tvDisplayUnits.setText(error);
                    return; }
            ...
            } tvDisplayUnits.setText(feedback); break;
            // If clear units button is clicked:
            case R.id.b_clear_units:
                SharedPreferences.Editor eMyUnits = spMyUnits.edit(); eMyUnits.remove("myUnits");
                eMyUnits.apply();
                break;
        } }
}